...
### Блок 1.5 — Дизайн и UX (запрос владельца 2026-09-23)
Полная переработка внешнего вида программы и сайта, приоритет — после закрытия
критических пунктов выше:
1. Новый дизайн SPA (`web/index.html`, `web/css/*.css`, `web/js/*.js`): современная
   тёмная/светлая тема, плавные анимации переходов и состояний, единая дизайн-система
   (токены цвета/типографики/радиусов/теней), микроанимации, скелетоны, тосты.
2. Новый дизайн лендинга (`site/`) в одном стиле с программой.
3. UX-фиксы: таблица не должна выходить за пределы экрана (горизонтальный скролл
   внутри контейнера + sticky-заголовки), запуск окна сразу в максимизированном
   режиме, корректное позиционирование/размер модальных окон, единый focus-trap.
4. Новые фишки (список согласовать с владельцем): командная палитра (есть
   `widgets/omnibox.py` как прототип), перетаскиваемые дашборд-плитки (есть
   `home_layout`), сохранённые виды/пресеты, «умный» поиск, горячие клавиши.
5. Игры 1-в-1: Blast-блок и 2048 — довести до полноценных копий
