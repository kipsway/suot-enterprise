...
| # | Место | Дефект | Статус |
|---|---|---|---|
| 1 | `server/routers/export_api.py:61-71` | Ветка `ids` читает записи без проверки владельца (соседняя ветка фильтров — с `owner_id`) | Подтверждено |
| 2 | `server/routers/print_pdf.py:192-196` | `batch_pdf` по `record_ids` без проверки владельца (ветка без ids — с изоляцией) | Подтверждено |
| 3 | `server/routers/print_api.py:259-282` | `preview` читает любую запись по id без проверки владельца (значения с 2.2.3 экранируются, но чтение чужого — открыто) | Подтверждено |
| 4 | `server/routers/custom.py:422-426` | `transfer` из системной таблицы читает чужие записи (ветка custom-таблиц проверяется через `_check_rec`) | Подтверждено |
| 5 | `server/routers/records.py:64-69,121-126` | Удаление чужих заметок/связей по id без проверки; `link_add` (:90-105) проверяет только исходную запись | Подтверждено |
| 6 | `server/routers/import_api.py:349-370` | Отмена чужого импорта по `import_id`; `history` отдаёт чужие; `STASH` глобальный без привязки к юзеру; `h['table_name']` интерполируется в SQL | Подтверждено |
| 7 | `server/routers/exporter_api.py:91-95` | `run_now` доступен любому залогиненному, экспорт идёт с `owner=None` (все данные), пишет файл на диск | Подтверждено |
| 8 | `server/routers/diag.py:76-197` | `summary/log/disk` (версии, пути, размеры, чужие события) — любому залогиненному, без `is_admin` | Подтверждено |
| 9 | `server/routers/calendar_api.py:297-310` | `update_category` делегирует `calendar_save_category(uid, ...)` — проверка внутри метода не прочитана | Требует проверки |
...
- CI: `.github/workflows/ci.yml` есть в HEAD, но удалён в рабочей копии —
  GitHub Actions без пайплайна; восстановить или подтвердить удаление.
...
3. Построчный IDOR-проход по разделу 7.2 (пункты 1-9): добавить проверки
   владельца (`_check_rec`/`_access`/owner-фильтры), закрыть `diag` под
   `is_admin`, привязать `STASH`/undo к пользователю; на каждый фикс —
   кейс в `tests/test_security_audit.py`.
4. `suot_platform.py` + `Dockerfile`: ревью диффов из 7.1, решение
   (freeze/delete/commit), затем точечный коммит (не `git add .`).
