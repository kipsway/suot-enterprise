...
TABLE_NAME = "training"
TRAINING_TYPES = ["Первичное", "Периодическое", "Внеплановое", "Целевое", "Стажировка"]
TRAINING_STATUSES = ["Активно", "Истекает", "Просрочено", "Архив"]


class TrainingEditDialog(QDialog, RecordTemplateMixin):
    def __init__(
        self,
        ...
        self.setWindowTitle(I18n._("training.edit") if data else I18n._("training.add"))
        self.setMinimumSize(680, 560)
        self.resize(820, 640)
        self._init_templates("training")
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        heading = QLabel(
            I18n._("training.edit") if self._data.get("id") else I18n._("training.add")
        )
        ...
        for col in self._columns:
            ...
            if typ == "Число":
                ...
                w = QComboBox()
                w.setMinimumHeight(36)
                if name == "Тип обучения" or name == "Тип":
                    w.addItems(TRAINING_TYPES)
                else:
                    w.addItems(TRAINING_STATUSES)
    ...
    def accept(self) -> None:
        errors = ValidationEngine.validate_dialog(
            self._fields, self._columns, "training"
        )
        if errors:
...
class TrainingTableWidget(
    ...
    def _build_ui(self) -> None:
        ...
        self._add_btn = GlassButton(I18n._("training.add"))
        ...
        add_export_buttons(
            toolbar, lambda: self._records, lambda: self._columns, "training", self
        )
    ...
    def _populate_filter(self) -> None:
        ...
        for r in self._all_records:
            s = str(r.get("data_json", {}).get("Статус", "")).strip()
    ...
    def _apply_filter(self) -> None:
        ...
        for r in self._all_records:
            dj = r.get("data_json", {})
            if status_text and status_text != I18n._("filter.all"):
                if str(dj.get("Статус", "")).strip() != status_text:
                    continue
            if search_text:
                found = any(
                    search_text in str(dj.get(c["name"], "")).lower()
    ...
    def _populate_table(self) -> None:
        ...
        for i, c in enumerate(self._columns):
            if c["name"] == "Срок действия" and c.get("visible", True):
                expiry_col = i
    ...
    def _on_header_clicked(self, idx: int) -> None:
        ...
        self._records.sort(
            key=lambda r: str(r.get("data_json", {}).get(col_name, "")),
        ...
        if dlg.exec_() == QDialog.Accepted:
            try:
                self.db.save_json_record(
                    TABLE_NAME, 0, dlg.get_data(), user_id=self._user_id
                )
                self._load_data()
    ...
    def _edit_record(self, record: Dict[str, Any]) -> None:
        dlg = TrainingEditDialog(record.get("data_json", {}), self._columns, self)
        if dlg.exec_() == QDialog.Accepted:
            try:
                self.db.save_json_record(
                    TABLE_NAME, record["id"], dlg.get_data(), user_id=self._user_id
                )
                self._load_data()
                ToastNotification.notify(I18n._("common.success"), "success", 3000)
            except Exception:
    ...
    def _delete_selected(self) -> None:
        ...
        if (
            QMessageBox.question(
                ...
                I18n._("training.delete_confirm"),
    ...
    def _export_pdf(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, I18n._("pdf.export"), f"training_{rec['id']}.pdf", "PDF (*.pdf)"
        )
        if not path:
            return
        rows_html = "".join(
        ...
        html = (
            f"<h2>{I18n._('tab.training')} #{rec['id']}</h2>"
    ...
    def _open_links(self) -> None:
        ...
        dlg = RecordLinksDialog("training", rec["id"], name, self)
        dlg.exec_()
    ...
    def _export_selected(self) -> None:
        ...
        dj = rec.get("data_json", {})
        path, _ = QFileDialog.getSaveFileName(
            self, I18n._("export.title"), f"training_{rec['id']}.csv", "CSV (*.csv)"
        )
        if not path:
            return
        try:
