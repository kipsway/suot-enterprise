...
class TrainingEditDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        heading = QLabel(I18n._("training.edit") if self._data.get("id") else I18n._("training.add"))
...
class TrainingTableWidget(QWidget, InlineEditMixin, ColumnWidthMixin):
    TABLE_NAME = "training"
    ...
    def _build_ui(self) -> None:
        ...
        self._add_btn = QPushButton(I18n._("training.add"))
        ...
        for r in self._all_records:
            ...
            if status_text and status_text != I18n._("filter.all"):
                if str(dj.get("Статус", "")).strip() != status_text:
                    continue
            if search_text:
                found = any(search_text in str(dj.get(c["name"], "")).lower() for c in self._columns)
                if not found:
                    continue
    ...
    def _delete_selected(self) -> None:
        ...
        if not rows:
            return
        if QMessageBox.question(self, I18n._("common.confirm"), I18n._("training.delete_confirm"),
    ...
    def _export_pdf(self) -> None:
        ...
        rows_html = "".join(
            ...
            for c in self._columns)
        html = (f"<h2>{I18n._('tab.training')} #{rec['id']}</h2>"
    ...
    def _export_selected(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(self, I18n._("export.title"),
                                               f"training_{rec['id']}.csv", "CSV (*.csv)")
        if not path:
            return
        try:
