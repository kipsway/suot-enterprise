...
class PPEEditDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        heading_text = I18n._("ppe.edit") if self._data.get("id") else I18n._("ppe.add")
        ...
        for col in self._columns:
            ...
            if typ == "Число":
                ...
                w.setMinimumHeight(36)
                if name == "Тип":
                    w.addItems(PPE_TYPES)
                else:
                    w.addItems(PPE_STATUSES)
...
class PPETableWidget(QWidget):
    ...
    def _populate_table(self) -> None:
        ...
        for i, c in enumerate(self._columns):
            if c["name"] == "Срок замены" and c.get("visible", True):
                expiry_col = i
    ...
    def _add_record(self) -> None:
        dlg = PPEEditDialog({}, self._columns, self)
    ...
    def _edit_record(self, record: Dict[str, Any]) -> None:
        ...
        dlg = PPEEditDialog(dj, self._columns, self)
        if dlg.exec_() == QDialog.Accepted:
            data = dlg.get_data()
            try:
