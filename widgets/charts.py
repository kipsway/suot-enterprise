...
class BarChart(QWidget):
    ...
    def set_data(
        self, data: List[Tuple[str, float]], title: str = "", color: str = "#2196F3"
    ) -> None:
        self._data = data
        self._title = title
    ...
    def paintEvent(self, event: Any) -> None:
        ...
        for i in range(steps + 1):
            ...
            painter.drawText(
                QRect(0, int(y) - 10, margin_left - 8, 20),
        ...
        for i, (label, value) in enumerate(self._data):
            ...
            if bar_h > 0:
                ...
                painter.drawRoundedRect(
                    QRect(int(x), int(y), int(bar_width), int(bar_h)), 3, 3
                )
            ...
            painter.setPen(text_color)
            label_rect = QRect(
                int(x) - 10, margin_top + chart_h + 4, int(bar_width) + 20, 40
            )
            painter.drawText(
                label_rect,
                Qt.AlignHCenter | Qt.AlignTop,
                label,
                QRect(1, 1, 1, 1) if len(label) > 10 else None,
            )
...
class PieChart(QWidget):
    ...
    def set_data(
        self, data: List[Tuple[str, float]], title: str = "", donut: bool = True
    ) -> None:
        self._data = data
        self._title = title
        self._donut = donut
    ...
    def paintEvent(self, event: Any) -> None:
        ...
        for i, (label, value) in enumerate(self._data):
            ...
            painter.drawPie(
                cx - radius, cy - radius, radius * 2, radius * 2, start_angle, span
            )
            if self._donut:
...
class TrendChart(QWidget):
    ...
    def set_data(
        self, points: List[Tuple[str, float]], title: str = "", color: str = "#2196F3"
    ) -> None:
        self._points = points
        self._title = title
    ...
    def paintEvent(self, event: Any) -> None:
        ...
        fill_poly.append(QPointF(margin_left + chart_w, margin_top + chart_h))
        painter.setPen(Qt.NoPen)
        ...
        for i in range(0, n, label_step):
            ...
            x = margin_left + chart_w * i / (n - 1)
            painter.drawText(
                QRect(int(x) - 30, margin_top + chart_h + 4, 60, 20),
                Qt.AlignCenter,
                label,
            )
