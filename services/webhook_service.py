...
WEBHOOK_EVENTS = [
    "violation.created", "violation.updated", "violation.overdue",
    "incident.created",
    "ppe.expiring", "ppe.issued",
    "training.expiring", "permit.expiring",
    "reminder.due",
]
...
def _send_webhook(url: str, payload: Dict[str, Any]) -> str:
    try:
        ...
        r = requests.post(url, json=payload, timeout=10,
                          headers={"User-Agent": "SUOT-Enterprise/1.0",
...
def save_target(target_id: int, name: str, url: str,
                events: str, enabled: bool) -> int:
    db = DatabaseManager()
    if target_id:
        db.execute(
